...
const Navigation = ({ isPageLoading, isInitialLoad = false, isScrolled = false, isModalOpen = false }) => {
  ...
  const getRouteFromTitle = (itemTitle) => {
    const routeMap = {
      ...
      "Materi": "/materi-pramuka",
      "Sejarah Pramuka": "/sejarah-pramuka",
      "Simpul & Ikatan": "/simpul-ikatan",
      "Sandi Pramuka": "/sandi-pramuka",
      "Peta": "/peta",
      "Tokoh Pramuka": "/tokoh-pramuka",
      "Fakta Jambore": "/fakta-jambore",
                ...
                {navigation.map((item, index) => (
                  <div key={item.title} className="relative">
                    ...
                    <AnimatePresence>
                      {activeDropdown === index && (
                        <motion.div
                          className="absolute top-full left-0 mt-2 w-56 bg-white/95 backdrop-blur-lg rounded-xl shadow-lg border border-gray-200/50 overflow-hidden"
