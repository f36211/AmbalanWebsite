...
import { BookOpen, ArrowRight, X, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react';
...
const CardSkeleton = ({ isMobile }) => (
  <div className={`${
    ...
    <div className={`${isMobile ? 'p-6' : 'p-5'} flex flex-col flex-1`}>
      <div className={`${isMobile ? 'h-6 mb-4' : 'h-5 mb-3'} bg-gray-300 rounded`}></div>
      <div className="space-y-2 flex-1">
        <div className="h-4 bg-gray-300 rounded"></div>
        <div className="h-4 bg-gray-300 rounded"></div>
        <div className="h-4 bg-gray-300 rounded w-3/4"></div>
      </div>
      <div className={`${isMobile ? 'h-12 mt-6' : 'h-10 mt-4'} bg-gray-300 rounded-xl`}></div>
    </div>
  </div>
);
...
const MateriPramukaPage = () => {
  ...
  const totalPages = useMemo(() => Math.ceil(materials.length / materialsPerPage), [materials.length, materialsPerPage]);
  const currentMaterials = useMemo(() => 
    materials.slice(currentPage * materialsPerPage, (currentPage + 1) * materialsPerPage),
                  ...
                  <div className={`${
                    ...
                    <div className={`relative ${isMobile ? 'h-56' : 'h-48'} overflow-hidden flex-shrink-0`}>
                      ...
                      <div className="absolute top-3 left-3">
                        ...
                        >
                          <span className={`text-white ${isMobile ? 'text-lg' : 'text-sm'} font-bold`}>
                            {material.id}
                          </span>
                        </motion.div>
                      </div>
                    ...
                    <div className={`${isMobile ? 'p-6' : 'p-5'} flex flex-col flex-1`}>
                      ...
                      <p className={`text-gray-600 ${
                        isMobile ? 'text-base mb-6 line-clamp-4' : 'text-sm mb-4 line-clamp-3'
                      ...
                      >
                        <BookOpen className={`${isMobile ? 'w-5 h-5' : 'w-4 h-4'} transition-transform group-hover/btn:scale-110`} />
                        Buka Materi
                        <ArrowRight className={`${isMobile ? 'w-5 h-5' : 'w-4 h-4'} transition-transform group-hover/btn:translate-x-1`} />
                      </motion.button>
                    </div>
                  </div>
                </motion.div>
        ...
        {selectedMaterial && (
          ...
          >
            ...
            >
              ...
              <div className={`${isMobile ? 'p-5 pr-16' : 'p-6 pr-16'} border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white`}>
                <motion.h2 
              ...
              <div className={`${isMobile ? 'p-4' : 'p-6'} overflow-auto ${
                ...
                >
                  ...
                  <div className="hidden flex-col items-center justify-center text-gray-500 p-8 text-center">
                    <BookOpen className={`${isMobile ? 'w-12 h-12' : 'w-16 h-16'} mb-4 text-gray-400`} />
                    <h3 className={`${isMobile ? 'text-lg' : 'text-xl'} font-semibold mb-2`}>
                      Materi {selectedMaterial.title}
                    </h3>
                    <p className={`${isMobile ? 'text-sm' : 'text-base'} leading-relaxed max-w-md`}>
                      {selectedMaterial.content || "Materi pembelajaran akan ditampilkan di sini."}
                    </p>
                  </div>
                </motion.div>
              </div>
